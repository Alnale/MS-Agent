...
const changelog: ChangelogEntry[] = [
  {
    ...
    latest: true,
    items: [
      { category: '新功能', text: '陪伴模式系统：全新情感状态追踪引擎，后端维护每会话的情绪/亲密度/精力/耐心/信任五维状态，Sentiment Agent 输出结构化 CompanionDelta 驱动状态演变，前端 CompanionPanel 实时展示 13 种情绪 Emoji（按强度分级）、亲密度五级标签、信任/耐心/精力进度条和思考气泡' },
      { category: '新功能', text: '子 Agent 协作时间线：助手消息底部新增可展开的 SubAgent 协作面板，以时间线形式展示每个子 Agent 的执行卡片，包含质量环指示器（高/中/低）、思考过程折叠、内容摘要和贴纸，卡片入场带渐变动画' },
      { category: '新功能', text: 'Agent 流水线可视化：ThinkingIndicator 重构为实时流水线阶段步进器，显示初始化→规划→执行→路由→合成五阶段节点（带勾选/发光指示），Agent 药丸实时展示运行/完成/失败状态，支持微光动画和阶段时长显示' },
      { category: '新功能', text: '模型思考过程展示：助手消息支持展开/折叠模型思考过程（thinking_delta），带时钟图标、复制按钮和动画过渡，深度思考内容与回复正文分离展示' },
      { category: '新功能', text: '背景切换过渡动画：切换背景时触发两段式过渡——色彩洗刷扫过（双层）、扩展环（3 环）、SVG 滴墨痕迹（6 条）和浮动微粒（6 颗），在动画中点执行实际背景替换' },
      { category: '新功能', text: '默认背景装饰元素：无自定义背景时渲染 5 个光晕球、3 个光圈、6 个星光点、2 个弧线和点阵网格覆盖层，提供丰富的默认视觉效果' },
      { category: '新功能', text: '视频主色智能检测：新增 useVideoDominantColor Hook，每 300ms 采样视频帧，基于中心加权高斯亮度分布计算最佳文字颜色（深/浅自适应），确保视频背景上文字可读性' },
      { category: '新功能', text: '视频背景自动解锁静音：首次用户交互（点击/按键）自动取消视频静音，跳过按钮点击避免冲突，尊重浏览器自动播放策略' },
      { category: '新功能', text: '消息内容分段渲染：MessageContentSegments 将渲染结果拆分为 HTML 和 Mermaid 段，通过事件委托处理复制和预览按钮，Mermaid 和 HtmlPreview 组件 React.lazy 懒加载' },
      { category: '新功能', text: 'HTML 预览增强：预览面板升级为 macOS 风格窗口（红黄绿交通灯按钮），新增刷新按钮（带旋转动画）和外部窗口打开功能，内容通过 DOMPurify 消毒防 XSS' },
      { category: '新功能', text: '工具选择器增强：工具定义缓存（60 秒 TTL），支持「直接执行」和「插入调用」两种模式，参数级联清除（修改父参数自动清除子参数），工具状态动画队列（成功/失败指示器显示 6 秒）' },
      { category: '新功能', text: '工具参数输入增强：布尔类型切换开关（带开关图标）、枚举类型下拉选择、整数数字输入、必填字段标记和描述提示，参数表单全面升级' },
      { category: '新功能', text: '工具循环去重与智能恢复：ReAct 工具循环检测已执行的工具+参数组合防止重复执行，LLM 重复调用已调用工具时自动跳出循环，错误恢复注入上下文提示（文件不存在→建议写入、权限问题→建议 /tmp/、超时→建议简化）' },
      { category: '新功能', text: '工具并行与依赖执行：支持 allow_parallel=true 的工具并行执行（join_all），depends_on 字段实现 DAG 风格依赖执行，依赖输出自动注入下游工具参数（如文件路径传递）' },
      { category: '新功能', text: '工具结果缓存：ToolResultCache 支持按工具类型配置 TTL（搜索 10 分钟、HTTP GET 5 分钟、POST 2 分钟），副作用工具（xxt/file/datetime）自动排除，FNV-1a 哈希语义缓存键，命中率统计' },
      { category: '新功能', text: '层级化记忆去重引擎：HierarchicalDedupEngine 支持同义词（0.92）/包含（0.85）/相关（0.70）三级相似度检测，自动推荐合并/跳过/替换/关联/新建五种去重动作，支持预计算嵌入避免冗余调用' },
      { category: '新功能', text: 'OpenAI Responses API 支持：新增 openai_responses 提供商配置，优先级最高，所有提供商统一包装 RetryProvider + CircuitBreakerProvider' },
      { category: '新功能', text: 'Per-Agent 深度思考配置：每个 Sub Agent（sentiment/task_planner/summary/critic）独立配置 Thinking 参数（enabled/budget_tokens/strategy），思考预算正确计入 max_tokens 计算' },
      { category: '新功能', text: '版本化 API 路由：统一 /v1/chat、/v1/health、/v1/tools、/v1/presets 规范接口，保留旧路由向后兼容，新增 Swagger UI（/v1/swagger-ui）和 OpenAPI 规范（/v1/openapi.json）' },
      { category: '新功能', text: '内置预设人格：config.json 支持 presets.builtin 配置内置人格（猫娘/程序员/故事家），GET /presets 端点返回内置和自定义人格列表' },
      { category: '新功能', text: 'Per-IP 速率限制：Token Bucket 令牌桶限流器，可配置 max_requests 和 window_secs，Axum 中间件集成（HTTP 429 响应），支持 X-Forwarded-For/X-Real-IP 提取，后台 5 分钟清理过期桶' },
      { category: '新功能', text: '环境变量配置解析：config.json 支持 ${VAR_NAME} 和 ${VAR_NAME:-default} 语法，单次遍历 O(n) 实现，缺少默认值时保留占位符并输出警告' },
      { category: '新功能', text: '批量事实提取：batch_extract_and_store_facts 单次 LLM 调用处理多条消息的事实提取，返回 source_index 追踪来源，效率远高于逐条提取' },
      { category: '新功能', text: '滑动窗口压缩：sliding_window_compress 保留最近 N 条对话（可配置 keep_recent），对较旧对话提取关键事实后归档，释放上下文空间' },
      { category: '新功能', text: '智能记忆修剪：smart_prune 多因子评分（权重/访问次数/时效/置信度/关系数/已确认状态），保留高频访问和高置信度记忆，软删除而非硬删' },
      { category: '新功能', text: 'LLM 矛盾消解：resolve_contradictions_with_llm 使用 LLM 分类记忆关系为 Supersedes/Contradicts/Related，批量处理单次 LLM 调用，失败时回退规则消解' },
      { category: '新功能', text: '记忆事件系统：MemoryEvent 枚举（TurnRecorded/FactConfirmed/SummaryProduced/UserFeedback/ContradictionDetected），MemoryEventHandler trait 支持可插拔事件处理器' },
      { category: '新功能', text: '会话过期保护：evict_stale_sessions 后台每 5 分钟清理闲置超 1 小时的会话指令和陪伴状态，50,000 会话上限防耗尽攻击' },
      { category: '新功能', text: 'Redis Lua 原子操作：touch/promote/update_quality/delete/archive 五个 Lua 脚本保证并发下的原子性读-改-写，防止更新丢失' },
      { category: '新功能', text: '优雅关停：Ctrl+C 信号处理，按序关闭 Telemetry 和服务，日志记录关停过程' },
      { category: '新功能', text: 'Docker 多阶段构建优化：依赖缓存层（先复制 manifest 构建依赖，再复制源码），最终镜像基于 distroless/cc-debian12（最小攻击面），config.json 注入容器' },
      { category: '新功能', text: '设置面板模块化重构：SettingsPanel 拆分为 InterfaceSettings、BubbleColorSettings、LiquidGlassSettings、CompanionSettings、BackgroundSettings 五个子组件，支持可折叠嵌套布局' },
      { category: '新功能', text: 'Liquid Glass 陪伴面板支持：设置面板新增第四标签页「情绪面板」，可独立开关陪伴面板的毛玻璃效果，App 根节点添加 data-lg-companion 属性' },
      { category: '新功能', text: 'Liquid Glass 按钮聚焦溶解：状态栏聚焦时，状态栏按钮、侧边栏切换和音乐入口按钮的玻璃效果平滑溶解/恢复，CSS transition 过渡' },
      { category: '优化', text: '上下文 Arc 包装 O(1) 克隆：AgentContext 大字段改用 Arc<Value>，clone_for_agent() 执行 O(1) 引用计数克隆而非深拷贝，自定义 arc_serde 模块支持序列化' },
      { category: '优化', text: '内存二级索引：内存存储新增 kind_index/session_index/hash_index/relation_index 四个二级索引，O(1) 按类型/会话/哈希/关系查询，复合索引交叉查询' },
      { category: '优化', text: 'Redis MGET 批量获取：所有 Redis 查询改用 MGET 批量获取，消除 N+1 查询模式，批量操作使用 Pipeline 单次往返' },
      { category: '优化', text: '热点缓存 LFU 驱逐：L1 热缓存按 access_count 最低优先驱逐非脏条目，额外驱逐 10% 留余量' },
      { category: '优化', text: '结构感知 JSON 压缩：compress_json_value 保留优先字段（success/status/error/path/url/count），大嵌套值智能截断（>2000 字符），数组保留前 3 后 1 元素' },
      { category: '优化', text: 'Token 预算感知记忆截断：truncate_by_token_budget 使用 estimate_tokens（中英文混合）按预算截断，超预算立即停止添加条目' },
      { category: '优化', text: '意图感知排序：RankingConfig::for_intent 按查询意图调整权重——历史召回提升时效性、偏好查询提升权重因子、任务延续提升 Summary 类型' },
      { category: '优化', text: '批量事实提取效率优化：batch_extract_and_store_facts 单次 LLM 调用处理多条消息，返回 source_index 追踪来源' },
      { category: '优化', text: '批量负反馈处理：record_negative_feedback 每次降低 30% 置信度和 50% 权重，设置最低阈值防止记忆完全消亡' },
      { category: '优化', text: '内存质量报告：generate_quality_report 返回总记忆数、平均置信度、平均权重、类型分布' },
      { category: '优化', text: '会话隔离防上下文泄漏：禁用跨会话用户画像检索和用户演进图谱，所有记忆查询限定 session_id + user_id 范围，Agent 输出仅存短期记忆' },
      { category: '优化', text: '流式 UI 节流：SSE chunk 每 33ms 批量刷新（~30fps），dirty flag + setTimeout 模式避免每次 chunk 触发重渲染' },
      { category: '优化', text: '设置批量持久化：300ms 防抖批量写入 localStorage，替代逐项单独写入' },
      { category: '优化', text: '音频分析器生命周期管理：AudioContext 随 bgVideo 变化创建一次不随播放/暂停销毁，Gain 节点统一静音控制，页面隐藏时暂停动画循环节省 CPU' },
      { category: '优化', text: 'Blob URL 内存泄漏防护：背景切换/卸载时主动撤销 blob URL，音乐加载时丢弃无效 blob URL，视频卡片卸载时撤销预览 blob' },
      { category: '优化', text: '反幻觉输入验证：Sentiment/TaskPlanner Agent 校验输入长度（2-10000 字符），拒绝可疑 emoji 模式，返回安全默认值而非 panic' },
      { category: '优化', text: 'JSON 提取三级回退：extract_json_from_response 支持直接解析→Markdown 代码块提取→花括号匹配三级回退，兼容 LLM 输出格式不一致' },
      { category: '优化', text: '请求体大小限制：RequestBodyLimitLayer 限制 1MB，CompressionLayer 响应压缩' },
      { category: '修复', text: '修复工具循环重复执行：executed_tool_keys HashSet 防止相同工具+参数重复执行，executed_tool_names 检测 LLM 重新调用已调用工具的 thinking-only 响应' },
      { category: '修复', text: '修复跨会话记忆污染：Agent 输出显式存储为 SHORT-TERM，仅从用户消息提取事实（防止 LLM 提取自身幻觉内容为"事实"），用户画像检索和演进图谱禁用' },
      { category: '修复', text: '修复 LLM 响应截断无感知：synthesis 响应命中 max_tokens 上限时发出警告，建议增加预算或简化 prompt' },
      { category: '修复', text: '修复空 LLM 响应崩溃：所有 Agent 处理空响应返回安全默认值（Knowledge Agent→"抱歉无法回答"，Response Agent→"抱歉无法生成回复"，ChangeDetector→空数组）' },
      { category: '修复', text: '修复播放器快速切换竞态：playIntentRef 追踪播放意图，防止快速播放/暂停切换时的竞态条件' },
      { category: '修复', text: '修复歌词 ASR 鲁棒性：重试机制（指数退避，最多 2 次），音频 Blob 验证（类型检查+大小 1KB-1GB），实际音频时长检测（非 API 报告时长），短行合并（<1s）和长行拆分（>8s），CJK 感知逐字符分行' },
      { category: '修复', text: '修复 XSS 安全漏洞：URL 净化拦截 javascript:/data:/vbscript:/jar: 协议，DOMPurify 消毒最终 HTML 输出，HTML 预览 iframe 内容消毒，工具标签从后端泄漏内容中剥离' },
    ],
  },
  {
    version: 'V1.5',
